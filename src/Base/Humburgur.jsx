...
export default function Humburgur() {
  ...
  return (
    <div className=" lg:hidden block z-40">
      ...
      <div
        className={`transform bg-slate-900 w-1/3 h-[900px] z-50 transition-all overflow-y-auto  fixed duration-300 ${
      ...
      >
        ...
        className="float-right hover:cursor-pointer"
          ...
          alt="multiply"
        />{" "}
        <ul className="flex pt-24 flex-col gap-5 text-sm mt-10 ">
          {data.map((item) => (
            <li key={item}>
              <Link
                onClick={() => handleSelected(item)}
                className=" text-white hover:bg-slate-200 rounded-lg px-9 py-2 hover:text-blue-900"
                to={item}
              >
                {t(`SideBar.${item}`)}
              </Link>
